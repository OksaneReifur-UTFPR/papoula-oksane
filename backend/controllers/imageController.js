...
const mostrarImagemNoBrowser = async (req, res) => {

    console.log('Requisição recebida para /view/:produtoId - Mostrar Imagem no Browser');
    ...
    try {
        ...
        console.error(`Erro ao buscar imagem para visualização (ID: ${produtoId}):`, error.message);
        
        // Opcional: Você pode enviar uma imagem "placeholder" (sem foto) aqui se quiser
        // ou apenas um erro 404
        res.status(404).json({ 
            message: `Imagem do produto ${produtoId} não encontrada.`,
            error: error.message 
        });
    }
};
...
module.exports = {
    ...
    uploadImage,
    viewImage,
    mostrarImagemNoBrowser // <--- Adicione aqui
};
